# script to execute CLP solving the LP relaxation using 
...
linesSumm=`cat relaxation-clp.csv | grep -i "${iname},"  | grep -i ",${params}," | wc -l`
...
rm -f "$outfile"
echo will run timeout ${timeLimit}s clp $1 keepnames off ${@:2} solu ${outfile} > /dev/null 2> ${errorfile}

utime=`{ time -p timeout ${timeLimit}s clp $1 ${@:2} solu ${outfile} > /dev/null 2> ${errorfile}; } 2>&1 >/dev/null | grep -i user | cut -d ' ' -f 2` 
    ...
    if grep -q "Status unknown" "$outfile"; then
        ...
        if grep -q "Stopped on iterations" "$outfile"; then
            ...
            if grep -q "Infeasible -" "$outfile"; then
                ...
                if [ ! "$fo" ]; 
                then
                    fo=`cat $outfile | grep -i "objective value"  | sed "s/  */ /g" | cut -d " " -f 3`
                fi
            fi
        fi
    fi
...
echo "${iname},${params},${utime},${fo}" >> relaxation-clp.csv
